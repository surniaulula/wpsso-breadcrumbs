 ...
 * Copyright 2018-2026 Jean-Sebastien Morisset (https://wpsso.com/)
 */
	...
	class WpssoBcBreadcrumb {
		...
		public static function add_breadcrumblist_data( array &$json_data, array $mods, $page_type_id ) {	// Pass by reference is OK.
			...
			if ( ! apply_filters( 'wpsso_bc_add_home_url', true ) ) {
				...
				$home_name = SucomUtilOptions::get_key_value( $key = 'bc_home_name', $wpsso->options, $mixed = 'current' );
			...
			if ( $wp_url === $home_url ) {
				...
				$wp_home_name = SucomUtilOptions::get_key_value( $key = 'bc_wp_home_name', $wpsso->options, $mixed = 'current' );
