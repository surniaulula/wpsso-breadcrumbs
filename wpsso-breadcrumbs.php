	...
	class WpssoBc {
		...
		public static function show_required_notices() {
			...
			foreach ( $info[ 'req' ] as $ext => $req_info ) {
				...
				$notice_msg = __( 'The %1$s add-on requires the %2$s plugin &mdash; install and activate the plugin or <a href="%3$s">deactivate this add-on</a>.', 'wpsso-breadcrumbs' );
		...
		public function wpsso_get_avail( $avail ) {
			...
			if ( ! $this->have_min_version ) {
				...
				$avail[ 'p_ext' ][ 'bc' ] = false;	// Signal that this extension / add-on is not available.

				return $avail;
			}

			$avail[ 'p_ext' ][ 'bc' ] = true;		// Signal that this extension / add-on is available.
